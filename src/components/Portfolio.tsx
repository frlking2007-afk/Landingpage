          ...
          {projects.map((project, index) => (
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-6">
                <span className="text-indigo-400 font-medium text-sm mb-2">{project.category}</span>
                <h3 className="text-white text-2xl font-bold mb-2">{project.title}</h3>
                <a href={project.link} className="inline-flex items-center text-white hover:text-indigo-300 transition-colors">
                  Ko'rish <ExternalLink className="ml-2 h-4 w-4" />
                </a>
              </div>
            </motion.div>
          ))}
